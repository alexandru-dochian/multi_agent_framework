...
def install_windows(auto_detect=False):
    def install_windows_with_powershell():
        ...
        run_cmd("(Invoke-WebRequest -Uri https://install.python-poetry.org -UseBasicParsing).Content | py -")
        # TODO
        ...
    ...
    if auto_detect:
        ...
        if is_wsl():
            ...
            raise Exception("Could not detect whether script is being run from wsl or powershell")

    else:
        choice = input("Specify installation agent: 'wsl' or 'powershell': ").strip().lower()
        if choice == 'wsl':
            install_windows_with_wsl()
